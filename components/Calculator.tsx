...
export default function Calculator(): JSX.Element {
  ...
  const fmtForCopy = (n: number, kind: 'flour' | 'water' | 'salt' | 'starter' | 'oil' | 'sugar'): string => {
    if (unit === 'oz') return ozFmt(n, kind === 'salt' ? 2 : 1);
    if (unit === 'cup') return fmt(n, kind, 'cup');
    return n.toFixed(0) + ' g';
  };
  ...
  const handleCopy = async (): Promise<void> => {
    const lines = [
      "Sourdough Recipe",
      "",
      `Flour (total):         ${fmtForCopy(flour, 'flour')}`,
      `Hydration:            ${hydration.toFixed(0)} %`,
      `Salt:                 ${fmtForCopy(saltGrams, 'salt')}  (${salt.toFixed(1)} % of flour)`,
      `Starter (total):      ${fmtForCopy(totalStarter, 'starter')}  (${starter.toFixed(0)} % of flour)`,
      `Olive oil:            ${fmtForCopy(oliveOilGrams, 'oil')}  (${oliveOil.toFixed(0)} % of flour)`,
      `Sugar:                ${fmtForCopy(sugarGrams, 'sugar')}  (${sugar.toFixed(0)} % of flour)`,
      "",
      "Other (added directly to dough):",
      `  Flour:              ${fmtForCopy(flourToAdd, 'flour')}  (excl. ${fmtForCopy(starterFlour, 'flour')} in starter)`,
      `  Water:              ${fmtForCopy(waterToAdd, 'water')}  (incl. ${fmtForCopy(starterWater, 'water')} in starter, total ${fmtForCopy(water, 'water')})`,
      `  Salt:               ${fmtForCopy(saltGrams, 'salt')}`,
      `  Starter:            ${fmtForCopy(totalStarter, 'starter')}`,
      "",
      `Total dough weight:   ${fmtForCopy(totalDough, 'flour')}  (${fmtSimpleGrams(totalDough, unit)})`,
      "",
